		 ...
		 <c:choose>
        	<c:when test="${login.userkind == '4' }">
		      ...
		      <div style="min-width: 320px; max-width: 800px;padding: 10px;border-radius: 7px;background: #ffffff;text-align:left">
						 <c:choose>
    				    	<c:when test="${login.userkind == '5' }">
     
							<input id="shtab1" type="radio" name="tabs" checked> 
							<label style="font-size:20px;" for="shtab1">문의하기 확인하기</label>
		 					</c:when>
		 				</c:choose>
		 
					<input id="shtab2" type="radio" name="tabs" checked>
					...
					<section id="content2" style="margin-bottom:20px;">
						<form method="POST">
							<p><input style="font-size:20px;border:none;border-right:0px; border-top:0px; boder-left:0px; border-bottom: 3px solid #4DABF7; width:600px;" required placeholder="카테고리 등록하세요" type="text" name="kind"></p>
