...
<div class="tour_wrap">
	...
	<div class="tour_wrap_in">
		<div style="height: 100px; border-bottom: 1px solid black;">
			<div class="quick-link">
				<ul class="link-list">
					...
					<c:forEach var="package_middle" begin="10" end="10" items="${pmdto }">
							${package_middle.name }
					</c:forEach></a> * </li>
					<li class="link-list-item"><a href="#">
					<c:forEach var="package_middle" begin="11" end="11" items="${pmdto }">
							${package_middle.name }
					</c:forEach></a> * </li>
					<li class="link-list-item"><a href="#">
					<c:forEach var="package_middle" begin="12" end="12" items="${pmdto }">
							${package_middle.name }
					</c:forEach></a> * </li>
					<li class="link-list-item"><a href="#">
					<c:forEach var="package_middle" begin="13" end="13" items="${pmdto }">
							${package_middle.name }
					</c:forEach></a> * </li>
					<li class="link-list-item"><a href="#">
					<c:forEach var="package_middle" begin="14" end="14" items="${pmdto }">
							${package_middle.name }
					</c:forEach></a></li>
				</ul>
			</div>
		...
		<div style="width: 300px; height: 1100px;">
			...
			<hr>
			<div style="width:300px; height: 800px;" id="tour_gyeongsang_filter" >
				...
				<hr>	
				<div style="width: 250px; margin-left: 20px;">
					<h3><a href="#" id="tour_gyeongsang_filter_1">
						<c:forEach var="package_middle" begin="10" end="10" items="${pmdto }">
							${package_middle.name }
						</c:forEach>
					</a></h3>
					<div style="width: 250px; height: 360px; background-color: #F2F2F2; display: none;"
							id="tour_gyeongsang_filter_1_in">
						<c:forEach var="package_detail" begin="56" end="66" items="${pddto }">
				...
				<hr>
				<div style="width: 250px; margin-left: 20px">
					<h3><a href="#" id="tour_gyeongsang_filter_2" >
						<c:forEach var="package_middle" begin="11" end="11" items="${pmdto }">
							${package_middle.name }
						</c:forEach>
					</a></h3>
					<div style="width: 250px; height: 240px; background-color: #F2F2F2; display: none;"
							id="tour_gyeongsang_filter_2_in">
						<c:forEach var="package_detail" begin="67" end="73" items="${pddto }">
							${package_detail.name }<hr>
						</c:forEach>
					</div>
				</div>
				<hr>
				<div style="width: 250px; margin-left: 20px">
					<h3><a href="#" id="tour_gyeongsang_filter_3" >
						<c:forEach var="package_middle" begin="12" end="12" items="${pmdto }">
							${package_middle.name }	
						</c:forEach>
					</a></h3>
					<div style="width: 250px; height: 60px; background-color: #F2F2F2; display: none;"
						...
						<c:forEach var="package_detail" begin="74" end="75" items="${pddto }">
							${package_detail.name }<hr>
						</c:forEach>
					</div>
				</div>
				<hr>
				<div style="width: 250px; margin-left: 20px">
					<h3><a href="#" id="tour_gyeongsang_filter_4" >
						<c:forEach var="package_middle" begin="13" end="13" items="${pmdto }">
							${package_middle.name }	
						</c:forEach>
					</a></h3>
					<div style="width: 250px; height: 30px; background-color: #F2F2F2; display: none;"
						...
						<c:forEach var="package_detail" begin="76" end="76" items="${pddto }">
							${package_detail.name }<hr>
						</c:forEach>
					</div>
				</div>
				<hr>
				<div style="width: 250px; margin-left: 20px">
					<h3><a href="#" id="tour_gyeongsang_filter_5" >
						<c:forEach var="package_middle" begin="14" end="14" items="${pmdto }">
							${package_middle.name }	
						</c:forEach>
					</a></h3>
					<div style="width: 250px; height: 30px; background-color: #F2F2F2; display: none;"
						...
						<c:forEach var="package_detail" begin="77" end="77" items="${pddto }">
							${package_detail.name }<hr>
						</c:forEach>
					</div>
				</div>
		...
		<div style="width: 900px; height: 2000px; background-color: #eaeaea; overflow-y: scroll;">
				...
				<div class="hbox paging">
					<c:if test="${map.paging.prev }">
						<a href="${cpath }/tour/tour_gyeongsang/${map.paging.begin - 1}?type=${param.type}&word=${param.word}">◀</a>
					</c:if>
				</div>
				<c:forEach begin="${map.paging.begin }" end="${map.paging.end }" var="i">
					<a href="${cpath }/tour/tour_gyeongsang/${i}?type=${param.type}&word=${param.word}">
				...
				<c:if test="${map.paging.next }">
					<a href="${cpath }/tour/tour_gyeongsang/${map.paging.end + 1}?type=${param.type}&word=${param.word}">▶</a>
				</c:if>
				<c:choose>
...
<script>
	/* 필터 보이기 */
	$("#tour_gyeongsang_filter > div").click(function(){
		event.preventDefault();
		const idx = (event.target.id).split('_').reverse()[0];
		resultDiv = document.getElementById('tour_gyeongsang_filter_' + idx + '_in');
