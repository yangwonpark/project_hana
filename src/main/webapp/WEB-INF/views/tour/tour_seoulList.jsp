...
<div class="tour_wrap">
		...
		<div style="height: 100px; border-bottom: 1px solid black;">
			<div class="quick-link">
				<ul class="link-list">
					...
					<c:forEach var="package_middle" begin="7" end="7" items="${pmdto }">
							${package_middle.name }
					</c:forEach></a> * </li>
					<li class="link-list-item"><a href="#">
					<c:forEach var="package_middle" begin="8" end="8" items="${pmdto }">
							${package_middle.name }
					</c:forEach></a> * </li>
					<li class="link-list-item"><a href="#">
					<c:forEach var="package_middle" begin="9" end="9" items="${pmdto }">
							${package_middle.name }
					</c:forEach></a></li>
				</ul>
			</div>
	...
	<div style="height: 1200px; display: flex;">
		<div style="width: 300px; height: 1100px;">
			...
			<hr>
			<div style="width:300px; height: 800px;" id="tour_seoul_filter" >
				...
				<div style="width: 250px; margin-left: 20px;">
					...
					<div style="width: 250px; height: 300px; background-color: #F2F2F2; display: none;"
							id="tour_seoul_filter_1_in">
						<c:forEach var="package_detail" begin="36" end="44" items="${pddto }">
				...
				<div style="width: 250px; margin-left: 20px">
					...
					<div style="width: 250px; height: 270px; background-color: #F2F2F2; display: none;"
							id="tour_seoul_filter_2_in">
						<c:forEach var="package_detail" begin="45" end="52" items="${pddto }">
							${package_detail.name }<hr>
						</c:forEach>
					</div>
				</div>
				...
				<div style="width: 250px; margin-left: 20px">
					...
					<div style="width: 250px; height: 90px; background-color: #F2F2F2; display: none;"
							id="tour_seoul_filter_3_in">
						<c:forEach var="package_detail" begin="53" end="55" items="${pddto }">
							${package_detail.name }<hr>
						</c:forEach>
					</div>
				</div>
		...
		<div style="width: 900px; height: 2000px; background-color: #eaeaea; overflow-y: scroll;">
				...
				<table id="tourList">
					...
					<tbody>
					<c:forEach var="tour_item" begin="0" end="55" items="${map.list }">
				...
				<div class="hbox paging">
					<c:if test="${map.paging.prev }">
						<a href="${cpath }/tour/tour_seoul/${map.paging.begin - 1}?type=${param.type}&word=${param.word}">◀</a>
					</c:if>
				</div>
				<c:forEach begin="${map.paging.begin }" end="${map.paging.end }" var="i">
					<a href="${cpath }/tour/tour_seoul/${i}?type=${param.type}&word=${param.word}">
				...
				<c:if test="${map.paging.next }">
					<a href="${cpath }/tour/tour_seoul/${map.paging.end + 1}?type=${param.type}&word=${param.word}">▶</a>
				</c:if>
				<c:choose>
...
<script>
	/* 필터 보이기 */
	$("#tour_seoul_filter > div").click(function(){
		event.preventDefault();
		const idx = (event.target.id).split('_').reverse()[0];
		resultDiv = document.getElementById('tour_seoul_filter_' + idx + '_in');
